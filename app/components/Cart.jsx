  ...
  return (
    <div className="flex flex-col">
      {/* <a href={checkoutUrl} target="_self">
        <button className="animated-button thar-three">
      ...
      <a
        href={checkoutUrl}
        target="_self"
        className="animated-button thar-three z-10"
        style={{
          fontFamily: 'Poppins-300',
        }}
      >
        <p className="uppercase text-center z-50 text-[14px] tracking-widest">
  ...
  return (
    <section aria-labelledby="summary-heading" className={summary[layout]}>
      ...
      <dl className="grid">
        ...
        >
          <Text as="dt" className={'uppercase'}>
            Total
          </Text>
...
function CartLineItem({line}) {
  ...
  return (
    <li key={id} className="flex gap-4">
      ...
      <div
        className="flex justify-between py-4 flex-grow w-full"
        style={{
          fontFamily: 'Poppins-200',
        }}
      >
        <div className="grid gap-1 w-full">
          ...
          <div className="grid gap-1">
            {(merchandise?.selectedOptions || []).map((option) => (
              <Text color="white" className={'text-xs'} key={option.name}>
                {option.value}
              </Text>
            ))}
          </div>
...
function ItemRemoveButton({lineIds}) {
  ...
  return (
    <fetcher.Form action="/cart" method="post">
      ...
      <button
        className="text-sm uppercase tracking-widest transition-all duration-200
        inline-block after:block after:w-[100%] after:bg-[#ffffff] after:mt-1 after:h-px after:transition-[width] after:duration-200 hover:after:w-0
        "
        type="submit"
      >
      ...
      <div className="flex items-center border">
        <fetcher.Form action="/cart" method="post">
          ...
          <button
            ...
            className="w-10 h-10 transition text-white disabled:cursor-not-allowed"
            value={prevQuantity}
            disabled={quantity <= 1}
          >
            <span>&#8722;</span>
        ...
        <div className="px-2 text-center" data-test="item-quantity">
          {loading ? (
            <div className="loader w-4 h-4 border-2 rounded-full border-[#D3AED2] border-t-2 border-t-transparent"></div>
          ) : (
            quantity
          )}
        </div>
        ...
        <fetcher.Form action="/cart" method="post">
          ...
          />
          <button
            className="w-10 h-10 transition text-white"
